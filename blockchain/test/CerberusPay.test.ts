...
describe("CerberusPay Tests", function () {
  ...
  it("Should do first payment", async function () {
    const { cerberusPay, cerberusCoin, cerberusAddress, owner, otherAccount } =
      await loadFixture(deployFixture);

    const instance = cerberusCoin.connect(otherAccount);
    await instance.approve(cerberusAddress, ethers.parseEther("0.01"));

    await expect(cerberusPay.pay(otherAccount.address)).to.emit(
      cerberusPay,
      "Paid"
    );
  });
  ...
  it("Should NOT do first payment", async function () {
    const { cerberusPay, cerberusCoin, owner, otherAccount } =
      await loadFixture(deployFixture);

    await expect(cerberusPay.pay(otherAccount.address)).to.be.revertedWith(
